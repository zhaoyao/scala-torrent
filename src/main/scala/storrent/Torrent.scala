...
object Torrent {
  ...
  implicit object TorrentBencodingProtocol extends DefaultBencodingProtocol {
    ...
    implicit val torrentFileFormat: BencodingFormat[TorrentFile] = new RootBencodingFormat[TorrentFile] {
      ...
      override def read(value: BcValue): TorrentFile = value.asBcDict.getFields("path", "length", "md5sum") match {
        case Seq(BcString(path), BcInt(length), BcNil) => TorrentFile(new String(path), length, None)
        case Seq(BcString(path), BcInt(length), BcString(md5sum)) => TorrentFile(new String(path), length, Some(md5sum))
  ...
  def apply(data: String): Try[Torrent] = {
    ...
    val metainfo = Try(raw.convertTo[Metainfo])
    metainfo.map(m => new Torrent(m, raw.asBcDict.getFields("info").head.asBcDict))
  }
...
case class Torrent private (metainfo: Metainfo, raw: BcDict) {
  ...
  val infoHashRaw = Util.sha1(raw.getFields("info").head.asBcDict().toString())
