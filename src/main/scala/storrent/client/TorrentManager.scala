...
import storrent.client.TorrentManager.{ StartSeeding, StartTorrent, TorrentIgnored, TorrentStarted }
...
class TorrentManager(torrentStore: ActorRef) extends Actor with ActorLogging {
  ...
  def createTorrentClient(torrent: Torrent): ActorRef = {
    val client = context.actorOf(Props(new TorrentSession(torrent, "file:///tmp")))
